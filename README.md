...
| 7 | Annual Open Source License | Annually | Không mã hóa | Unlimited | `MJET-PC-UEN-` | Khi còn active subscription |
| 8 | Lifetime Open Source License | One Time | Không mã hóa | Unlimited | `MJET-PC-UEN-` | **Lifetime** (bao gồm) |

> **Ghi chú về "Open Source":** Đây KHÔNG phải open source. License vẫn là thương mại (proprietary). Khách mua quyền sử dụng source code không mã hóa, có thể đọc/tùy chỉnh nhưng **không được phân phối lại**. Tên "Open Source" thay vì "Open Source" để tránh nhầm lẫn với giấy phép OSS (GPL, MIT...).
...
**Giải thích giá Open Source cao hơn:** Khách nhận source code đọc được + quyền tùy chỉnh + white-label. Đây là giá trị premium so với bản encoded.
...
└── Open Source Licenses
    ...
    └── PC Lifetime Open Source ($299.95 one-time)
```
...
**Open Source License (products #7–8):**
...
| `modulejet-pricing-calculator-Open Source-v1.2.0.zip` | Open Source #7–8 | ✅ |
...
class LicenseChecker
{
    ...
    public function getTier(): string
    {
        ...
        if (str_starts_with($this->licenseKey, 'MJET-PC-UEN-')) return 'Open Source';
        return 'unknown';
    }
...
| Any Encoded → Open Source | Different distribution | WHMCS Product Upgrade (prorate) |
| Open Source Annual → Lifetime | Same tier, longer cycle | WHMCS Product Upgrade |
...
### 7.2 Build script — Open Source version
...
SRC="./src/modulejet_pricing_calculator"
OUT="./build/Open Source/modulejet_pricing_calculator"
VERSION="1.2.0"
...
zip -r "../modulejet-pricing-calculator-Open Source-v${VERSION}.zip" \
  modulejet_pricing_calculator/
```
...
| 2.7 | Setup download manager (2 packages: encoded + Open Source) | High | 0.5 ngày |
...
| 3.3 | Tạo build-Open Source.sh script | High | 0.5 ngày |
...
│   └── modulejet-pricing-calculator-Open Source-v1.2.0.zip
│
├── scripts/
│   ├── build-encoded.sh
│   └── build-Open Source.sh
...
- [ ] Terms of Service (bao gồm: Open Source ≠ open source, cấm redistribute)
- [ ] Privacy Policy
...
| Bản Open Source bị redistribute | Medium | Proprietary license + DMCA enforcement + community trust |
