...
- Mỗi sản phẩm có 3 tiers (Standard / Professional / Open Source) × 3 cycles (Monthly / Annual / Lifetime) = tối đa 8 SKU
...
{PRODUCT}   2–4 uppercase letters — mã sản phẩm
{TIER}      STD / PRO / UEN
...
|---|---|---|
| `PC` | Pricing Calculator | Sản phẩm đầu tiên |
| `IM` | Invoice Manager | Planned |
| `SM` | Server Manager | Planned |
| `EM` | Email Manager | Idea |
| `DM` | Domain Manager | Idea |
| `BK` | Backup Manager | Idea |

Khi tạo sản phẩm mới, đăng ký mã 2–4 ký tự vào registry này để tránh trùng.
...
| Open Source | `UEN` | Plain PHP | Unlimited | ✅ | ❌ | ✅ | Priority 24h |
...
| 2 | `{Product} Annual Standard` | Annually | STD |
| 3 | `{Product} Lifetime Standard` | One Time | STD |
| 4 | `{Product} Monthly Professional` | Monthly | PRO |
| 5 | `{Product} Annual Professional` | Annually | PRO |
| 6 | `{Product} Lifetime Professional` | One Time | PRO |
| 7 | `{Product} Annual Open Source` | Annually | UEN |
| 8 | `{Product} Lifetime Open Source` | One Time | UEN |
...
|---|---|---|---|
| Standard | `1×` base | `8× base` (~33% off yearly) | `20× base` |
| Professional | `2× base` | `16× base` | `40× base` |
| Open Source | — | `24× base` | `60× base` |

**Ví dụ base = $4.95:**

| Tier | Monthly | Annual | Lifetime |
|---|---|---|---|
| Standard | $4.95 | $39.95 | $99.95 |
| Professional | $9.95 | $79.95 | $199.95 |
| Open Source | — | $119.95 | $299.95 |

Sản phẩm phức tạp hơn (Server Manager) có thể dùng base cao hơn ($7.95, $9.95...).

### 4.2 Support/Updates Policy (global)

| Billing | Policy |
|---|---|
| Monthly / Annual | Bao gồm khi subscription active. Hết hạn = mất download + support |
...
□ Đăng ký product code vào registry (e.g. IM = Invoice Manager)
□ Tạo Product Group: "ModuleJET — {Product Name}"
□ Tạo 8 products theo SKU matrix (section 3.5)
□ Set key prefix cho mỗi product: MJ-{CODE}-STD- / MJ-{CODE}-PRO- / MJ-{CODE}-UEN-
□ Set domain rules: STD = no domain conflict, PRO/UEN = allow domain conflict
□ Set pricing theo framework (section 4.1)
□ Cấu hình upgrade paths (STD→PRO, Monthly→Annual→Lifetime, Encoded→Open Source)
...
│   ├── PC Lifetime Standard ($99.95)
│   ├── PC Monthly Professional ($9.95/mo)
│   ├── PC Annual Professional ($79.95/yr)
│   ├── PC Lifetime Professional ($199.95)
│   ├── PC Annual Open Source ($119.95/yr)
│   └── PC Lifetime Open Source ($299.95)
│
...
| Open Source ZIP | UEN products | `mj-{product}-Open Source-v{version}.zip` |
...
```
Within same product:
  STD Monthly  →  STD Annual  →  STD Lifetime
  PRO Monthly  →  PRO Annual  →  PRO Lifetime
  UEN Annual   →  UEN Lifetime
  
Cross-tier:
  STD (any cycle)  →  PRO (any cycle)
  STD (any cycle)  →  UEN (any cycle)
  PRO (any cycle)  →  UEN (any cycle)
...
# ── Open Source build ─────────────────────────────
OUT_UEN="./build/Open Source/${MODULE}"
rm -rf "./build/Open Source" && mkdir -p "$OUT_UEN"
cp -r "$SRC/"* "$OUT_UEN/"
cp ./licenses/COMMERCIAL.txt "$OUT_UEN/LICENSE"

cd "./build/Open Source"
zip -r "../mj-${ABBR}-Open Source-v${VERSION}.zip" "${MODULE}/"
cd ../..

echo "✅ Encoded:   build/mj-${ABBR}-encoded-v${VERSION}.zip"
echo "✅ Open Source:  build/mj-${ABBR}-Open Source-v${VERSION}.zip"
```
...
├── Pricing Calculator Support      ← tạo khi launch PC
├── Invoice Manager Support         ← tạo khi launch IM
├── Server Manager Support          ← tạo khi launch SM
...
| 1.5 | Tạo 8 products trong WHMCS (MJ-PC-STD/PRO/UEN) | 0.5 ngày |
...
│   ├── mj-pricing-Open Source-v1.2.0.zip
...
| Support response time (PRO/UEN) | < 24h |
| Refund rate | < 10% |
...
- [ ] Product code registered in registry
...
- [ ] Domain rules set (STD=restricted, PRO/UEN=unlimited)
...
| Key format | `MJ-{PRODUCT}-{TIER}-{random32}` |
| Tiers | Standard (1 domain, encoded) / Professional (unlimited, encoded) / Open Source (unlimited, plain PHP) |
...
*ModuleJET Platform Licensing Plan v1.0 — March 2026*  
